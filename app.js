    ...
    if (faqContainer && typeof siteData !== 'undefined') {
        if(siteData.faq.length === 0) {
            ...
            siteData.faq.forEach((item, index) => {
                ...
                faqItem.className = 'bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300';
    ...
    if (testsContainer && typeof siteData !== 'undefined') {
        if(siteData.tests.length === 0) {
            ...
            siteData.tests.forEach((test) => {
                ...
                testCard.className = 'bg-white hover:bg-blue-50 transition-colors duration-200 p-4 md:p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col md:flex-row items-start md:items-center justify-between group';
                ...
                testCard.innerHTML = `
                       ...
                       <div class="flex space-x-3 w-full md:w-auto mt-2 md:mt-0">
                          <a href="${test.testUrl || test.url || '#'}" target="_blank" class="flex-1 md:flex-none text-center px-6 py-2.5 bg-gray-900 text-white font-medium rounded-lg hover:bg-primary transition-colors shadow-sm">Arkusz PDF</a>
